...
function switchHookTab(hookType) {
    ...
    document.querySelectorAll('.hook-tab').forEach(tab => {
        const isActive = tab.dataset.hook === hookType;
        tab.classList.toggle('active', isActive);
    });
    ...
    const panelId = `hookPanel${hookType.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('')}`;
...
function loadHooksConfig(hooksConfig) {
    ...
    hookTypes.forEach(({ key, prefix }) => {
        ...
        const scriptTypeEl = document.getElementById(`${prefix}ScriptType`);
        const scriptEl = document.getElementById(`${prefix}Script`);
        const timeoutEl = document.getElementById(`${prefix}Timeout`);
        
        if (enabledEl) enabledEl.checked = config.enabled || false;
        if (scriptTypeEl) scriptTypeEl.value = config.script_type || 'python';
        if (scriptEl) scriptEl.value = config.script || '';
        if (timeoutEl) timeoutEl.value = config.timeout || 30;
    });
}
...
async function viewHookLogsModal() {
    ...
    try {
        const response = await fetch(`${API_BASE}/tasks/${taskId}/hooks/logs?per_page=20`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (!response.ok) {
            throw new Error('获取钩子日志失败');
        }

        const data = await response.json();
        
        if (!data.logs || data.logs.length === 0) {
            showNotification('暂无钩子执行日志', 'info');
            return;
        }
        ...
        // 构建日志显示内容
        let logSummary = `共 ${data.logs.length} 条记录:\n\n`;
        data.logs.slice(0, 5).forEach((log, index) => {
            const statusEmoji = log.status === 'success' ? '✅' : log.status === 'failed' ? '❌' : '⏱️';
            logSummary += `${statusEmoji} ${log.hook_type}\n`;
            logSummary += `   时间: ${new Date(log.execution_start).toLocaleString()}\n`;
            logSummary += `   耗时: ${log.execution_duration ? log.execution_duration.toFixed(2) + 's' : '-'}\n`;
            if (log.error_message) {
                logSummary += `   错误: ${log.error_message.substring(0, 100)}...\n`;
            }
            logSummary += `\n`;
        });
        
        if (data.logs.length > 5) {
            logSummary += `... 还有 ${data.logs.length - 5} 条记录（查看控制台）`;
        }
        
        showNotification(logSummary, 'info');
        
    } catch (error) {
        console.error('获取钩子日志失败:', error);
        showNotification('获取钩子日志失败: ' + error.message, 'error');
    }
}
 ...
 * 查看任务钩子执行日志（外部调用）
 */
async function viewHookLogs(taskId) {
    if (!taskId) return;
    
    try {
        const response = await fetch(`${API_BASE}/tasks/${taskId}/hooks/logs?per_page=50`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (data.logs && data.logs.length > 0) {
            showNotification(`已输出 ${data.logs.length} 条钩子日志到控制台`, 'success');
        } else {
            showNotification('暂无钩子执行日志', 'info');
        }
    } catch (error) {
...
window.testCurrentHook = testCurrentHook;
window.viewHookLogs = viewHookLogs;
window.viewHookLogsModal = viewHookLogsModal;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (versionInfo && data.version) {
                // 检查是否为dev版本（包含dev关键字）
                const isDev = data.version.toLowerCase().includes('dev');
                ...
                if (isDev) {
                    ...
                    devBadge.style.cssText = 'margin-left: 6px; padding: 2px 6px; background: #ff6b6b; color: white; font-size: 10px; border-radius: 3px; font-weight: bold;';
                    versionInfo.appendChild(devBadge);
                }
